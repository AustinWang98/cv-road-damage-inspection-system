...
| Google Drive data handoff | Member 2 | Validated: `Member 2 data contract validated.` | Google Drive + Colab |
...
Two capacity-matched single-stage detectors on Part A's shared split, 640 px, batch 16, seed 42, 30 epochs, NVIDIA L4, Ultralytics 8.4.117. Confidence thresholds were selected on validation (`conf*` 0.228 / 0.237) and the 5,783-image test set was evaluated once.

| Test metric | YOLO11n | YOLO11s |
|---|---:|---:|
| mAP@0.50 | 0.4336 | **0.4439** |
| mAP@0.50:0.95 | 0.2033 | **0.2080** |
| F1 @ conf* | 0.4627 | **0.4783** |
...
Artifacts live in Drive under `RDD2022_Project/member2_runs/` (`tables/*.csv`, `figures/*.png`, per-run weights and Ultralytics plots) and `RDD2022_Project/member2_configs/` (hash-pinned training manifests).
